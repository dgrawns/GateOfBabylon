...
SWEP.PrintName = "Gate of Babylon (Explosions)"
SWEP.Slot	   = 1
SWEP.SlotPos   = 1
...
function SWEP:PrimaryAttack()
	...
	local ent = ents.Create("gold_weapon")
	-------------------------------------------------
	if IsValid(ent) then
		-------------------------------------------------
		ent:SetModel(table.Random(Weapon_Table))
		ent:SetMaterial("models/player/shared/gold_player")
		util.SpriteTrail(ent, 0, Color(218, 165, 32), false, 2, 1, 1, 0.5, "trails/laser.vmt")
		ent:PhysicsInit(SOLID_VPHYSICS)
		ent:SetPos(self.Owner:GetPos() + Vector(math.random(-300, 300),math.random(-300, 300),math.random(80, 300)))
		...
		timer.Simple(1, function()
			...
			ent:EmitSound("weapons/cbar_miss1.wav", 75, 150, 1, CHAN_AUTO)
			phys:ApplyForceOffset(velocity, push)
			ent:SetOwner(self.Owner)
			ent:Input("SetTimer", self:GetOwner(), self:GetOwner(), 1.5)
		end)
		-------------------------------------------------
		...
		ent:AddCallback("PhysicsCollide", Collide)
	end
end 
...
function SWEP:SecondaryAttack()
	...
	if IsValid(ent) then
	-------------------------------------------------
		ent:SetModel(table.Random(Weapon_Table2))
		ent:SetMaterial("models/player/shared/gold_player")
		util.SpriteTrail(ent, 0, Color(218, 165, 32), false, 2, 1, 1, 0.5, "trails/laser.vmt")
		ent:PhysicsInit(SOLID_VPHYSICS)
		ent:SetPos(self.Owner:GetPos() + Vector(0, 0, 140))
		ent:SetAngles(self.Owner:EyeAngles())
		ent:Spawn()
		ent:SetGravity(0)
		-------------------------------------------------
	...
	ent:AddCallback("PhysicsCollide", Collide3)
	-------------------------------------------------
end
